...
class ElectricBorder {
  constructor(element, options) {
    ...
    this.glowBlur     = o.glowBlur     !== undefined ? o.glowBlur     : 12;
    this.borderOffset = o.borderOffset !== undefined ? o.borderOffset : 60;
    this.displacement = o.displacement !== undefined ? o.displacement : 60;

    /* Noise constants — faithful to original */
    this._OCTAVES    = 10;
  ...
  _frame(ts) {
    ...
    var self  = this;
    var delta = this._lastTs ? (ts - this._lastTs) / 1000 : 0;
    ...
    for (var i = 0; i <= n; i++) {
      ...
      var xn = this._octNoise(p * 8, this._time, 0) * this.displacement;
      var yn = this._octNoise(p * 8, this._time, 1) * this.displacement;
  ...
  /* ── Pause / resume (for hover-only borders) ───────────────── */
  pause() {
    this._paused = true;
    if (this._raf) { cancelAnimationFrame(this._raf); this._raf = null; }
    /* Clear the canvas so nothing is visible while idle */
    if (this._ctx) this._ctx.clearRect(0, 0, this._W * this._dpr, this._H * this._dpr);
  }
