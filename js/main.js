...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* --- Portfolio card stack shuffle --- */
  (function () {
    var stack = document.querySelector('.work-stack');
    if (!stack) return;

    var cards = Array.prototype.slice.call(stack.querySelectorAll('.work-card'));
    var dots  = Array.prototype.slice.call(document.querySelectorAll('.work-stack-dot'));
    var n     = cards.length;
    var cur   = 0;
    var timer = null;

    function render(exitCard, exitDir) {
      cards.forEach(function (card, i) {
        card.classList.remove('wc-active', 'wc-behind-1', 'wc-behind-2', 'wc-exit-left', 'wc-exit-right');
        if (card === exitCard) {
          card.classList.add(exitDir >= 0 ? 'wc-exit-left' : 'wc-exit-right');
        } else if (i === cur) {
          card.classList.add('wc-active');
        } else if (i === (cur + 1) % n) {
          card.classList.add('wc-behind-1');
        } else if (i === (cur + 2) % n) {
          card.classList.add('wc-behind-2');
        }
      });
    ...
    function go(dir, target) {
      var old = cards[cur];
      cur = (target !== undefined) ? ((target % n + n) % n) : ((cur + dir + n) % n);
      render(old, dir);
      setTimeout(function () {
        old.classList.remove('wc-exit-left', 'wc-exit-right');
      }, 580);
    }
    ...
    // Buttons
    var prevBtn = document.querySelector('.wc-prev');
    var nextBtn = document.querySelector('.wc-next');
    ...
    var touchX = 0;
    stack.addEventListener('touchstart', function (e) { touchX = e.touches[0].clientX; }, { passive: true });
    stack.addEventListener('touchend', function (e) {
      var dx = e.changedTouches[0].clientX - touchX;
      if (Math.abs(dx) > 48) { go(dx < 0 ? 1 : -1); startTimer(); }
    });
  })();
